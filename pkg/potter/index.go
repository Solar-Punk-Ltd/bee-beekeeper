...
func New(mode pot.Mode, logger log.Logger) (*Index, error) {
	idx := &Index{
		logger: logger,
		mode:   mode,
...
func NewReference(mode pot.Mode, ref []byte, logger log.Logger) (*Index, error) {
	idx := &Index{
		logger: logger,
		mode:   mode,
		read:   make(chan pot.Node),
	...
	if !loaded {
		idx.logger.Info("root not loaded from persistent storage")
