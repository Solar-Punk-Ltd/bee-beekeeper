...
type ActMock struct {
	AddFunc    func(key []byte, val []byte) dynamicaccess.Act
...
func (act *ActMock) Add(key []byte, val []byte) dynamicaccess.Act {
	if act.AddFunc == nil {
		return act
	}
...
func NewActMock(addFunc func(key []byte, val []byte) dynamicaccess.Act, getFunc func(key []byte) []byte) dynamicaccess.Act {
	return &ActMock{
		AddFunc:    addFunc,
		LookupFunc: getFunc,
	}
