...
func getHistoryFixture(t *testing.T, ctx context.Context, ls file.LoadSaver, al dynamicaccess.ActLogic, publisher *ecdsa.PublicKey) (swarm.Address, error) {
	...
	al.AddPublisher(kvs0, publisher)
	kvs0Ref, _ := kvs0.Save()
	kvs1 := kvs.New(ls, swarm.ZeroAddress)
	al.AddGrantee(kvs1, publisher, &pk1.PublicKey, nil)
	al.AddPublisher(kvs1, publisher)
	kvs1Ref, _ := kvs1.Save()
	kvs2 := kvs.New(ls, swarm.ZeroAddress)
	al.AddGrantee(kvs2, publisher, &pk2.PublicKey, nil)
	al.AddPublisher(kvs2, publisher)
	kvs2Ref, _ := kvs2.Save()
